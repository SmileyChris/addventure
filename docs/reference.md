...
| `image_style` | Non-edge visual style for the title page image, such as `border`, `scratches`, `sepia`, or `greyscale` |
| `image_edge` | Edge treatment for the title page image, such as `torn-edge` or `burned-edge` |
...
| `"Room Name"` | Move the player to another room. Use `"Room__"` (trailing `__`) to explicitly target the base state |
| `ENTITY__STATE` | Transform object to a new state |
| `room -> room__STATE` | Transform the current room to a new state |
| `room -> room__` | Revert the current room to its base state (no state suffix) |
